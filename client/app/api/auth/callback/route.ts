...
import type { OIDCState } from '@/lib/auth'
...
export async function GET(request: NextRequest) {
  ...
  const raw = request.cookies.get('oidc-state')?.value
  const oidcState = raw
    ? (JSON.parse(decodeURIComponent(raw)) as OIDCState)
    : ({} as OIDCState)
  ...
  const redirectTo = oidcState.next
    ? decodeURIComponent(oidcState.next)
    : '/profile'
